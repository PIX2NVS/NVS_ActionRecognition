...
tf.app.flags.DEFINE_string('data_dir', os.path.abspath('/data0/test_data_allp_preconv'),
                           """Data directory""")
tf.app.flags.DEFINE_string('out_dir', os.path.abspath('/data0/Aaron/JM_all_P_frames_bin_with_index'),
...
tf.app.flags.DEFINE_string('path_to_file_list', None,
                           """Path to list of filenames to parse (set to 'None' to train on everything in data_dir)""")

#os.path.join(os.path.curdir, 'ucfTrainTestlist', 'trainlist01.txt')
...
for file in finput:
    ...
    for i in range(0, len(data), frame_bytes):
        ...
        frameIndex += 1
        frame_index_bytes = struct.pack('i', frameIndex)
        #frame_index_bytes = data[i + 4:i + 8]

        if True: #frame_type == 'P':
            with open(bin_file, 'ab') as b:
                bytes = frame_index_bytes + mv_bytes
                b.write(bytes)
            count += 1
